...
class MessageValidationApiTest {
  ...
  void emptyMessageIsRejected(APIRequestContext request) {
    ApiResult response =
        new MessageClient(request).sendMessage(new MessageRequest(null, null, null, null, null));
  ...
  void malformedEmailIsRejected(APIRequestContext request) {
    ApiResult response =
  ...
  void tooShortSubjectAndMessageAreRejected(APIRequestContext request) {
    ApiResult response =
        new MessageClient(request)
            .sendMessage(
                new MessageRequest(
  ...
  void tooShortPhoneIsRejected(APIRequestContext request) {
    ApiResult response =
        new MessageClient(request)
            .sendMessage(
                new MessageRequest(
    ...
    assertThat(response.status()).isEqualTo(400);
    assertThat(response.bodyAsStringList()).contains("Phone must be between 11 and 21 characters.");
  }
}
