 ...
 * intermediate "authenticated but not admin" outcome. A non-allowlisted GitHub account never
 * receives any {@code SecurityContext}/session at all: Spring Security treats an exception thrown
 * from an {@link OAuth2UserService} as an authentication failure, routed to the login failure
 * handler, before any session is ever established.
 *
 * <p>Compares only GitHub's immutable numeric {@code "id"} attribute against the injected {@link
 * AdminGithubAllowlist} - never {@code "login"} (the username, which can change and is read only
 * for display). A missing or non-numeric {@code "id"}, or an unexpected/malformed response from
 * GitHub, is always treated as a failed login, never a best-effort guess.
 *
 * <p>Only ever constructed when GitHub OAuth2 is enabled - it depends on {@link
 * AdminGithubAllowlist}, itself conditional on the same property, so the two must always be created
 * (or not) together, otherwise the permissive/no-credentials-configured case would fail to start
...
public class GithubOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
  ...
  public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
    ...
    try {
      ...
      // not the error's own description - the 3-arg form is what actually surfaces this message.
