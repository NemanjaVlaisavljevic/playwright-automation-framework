...
- [ ] **Revert `f8c093d` and re-confirm green — not done yet.** Verified via a fresh `git fetch` against the real GitHub remote that `master` is still at `f8c093d` as of this writing: the deliberate failing assertion in `NotFoundRunE2eTest` is still live on `master`, unreverted. Do not treat this phase as closed until the revert is committed, pushed, `dashboard-e2e.yml` re-run green, and that run ID recorded here.

`runner-dashboard-v1.0` sign-off is **not yet complete** — everything above is proven except the final revert-and-reconfirm step.
